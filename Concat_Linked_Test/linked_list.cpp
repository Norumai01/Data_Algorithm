...
public:
	...
	void removeFront();
	void currEle(SLinkedList<T>& list, int element);
	int size();
	friend void concat<T>(SLinkedList<T>& l1, SLinkedList<T>& l2, SLinkedList<T>& lout); 
private:
	SNode<T>* head;
...
template <typename T>
void SLinkedList<T>::currEle(SLinkedList<T>& list, int element) {
    if (list.empty()) {
        return;  // Shouldn't do anything, if the Linked list is empty.
    }
...
int main() {
	...
	p3.currEle(p3, 3);    // Should print Java, but not removes the previous elements.
